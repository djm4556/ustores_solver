...
def order(stage_cols: list, cols: list) -> list:
    ...
    if (1 <= cols.index("B") <= 3 and 1 <= cols.index("Y") <= 3) or (  # If B and Y are both on the left...
            5 <= cols.index("B") <= 7 and 5 <= cols.index("Y") <= 7):  # or both on the right, swap B with...
        swap_int(stage_cols, stage_cols.index("B"), 5 - stage_cols.index("B"))  # its opposite in the list.
    if 5 <= cols.index("R") <= 7:  # If red is on the right...
        swap(stage_cols, "R", "Y")  # Swap red and yellow.
    if 1 <= cols.index("B") <= 3:  # If blue is on the left...
